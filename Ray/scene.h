...
namespace Ray
{
	...
	public:
		...
		*** or the contribution from subsequent bounces is guaranteed to be less than the cut-off. */
		Util::Point3D getColor( Util::Ray3D ray , int rDepth , Util::Point3D cLimit , unsigned int lightSamples , bool binaryShadows , unsigned int tIdx );

		/** This method ray-traces the scene and returns the computed image */
		Image::Image32 rayTrace( int width , int height , int rLimit , double cLimit , unsigned int lightSamples , bool binaryShadows , bool showProgress );
