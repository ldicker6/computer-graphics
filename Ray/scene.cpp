...
Image32 Scene::rayTrace( int width , int height , int rLimit , double cLimit , unsigned int lightSamples , bool binaryShadows , bool showProgress )
{
	...
	auto RayTraceFunction = [&]( unsigned int threadIndex , size_t pixelIndex )
	{
		...
		try
		{
			...
			Point3D c = getColor( ray , rLimit , Point3D( cLimit , cLimit , cLimit ) , lightSamples , binaryShadows , threadIndex );
			Pixel32 p;
