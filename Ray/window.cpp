...
void Window::SetEvaluators( void )
{
	...
	case RotationParameters::QUATERNION:
		...
		break;
	default:
		THROW( "unsupported parametrization type: " , parametrizationType );
	}
}
...
void Window::MainMenu( int entry )
{
	...
	{
	case QUIT: exit(0);
	case RAY_TRACE_VIDEO:
	{
		auto ReadInput = []< typename T >( T & t )
		{
			std::string line;
			std::getline( std::cin , line );
			if( !line.length() ) return false;
			std::stringstream sStream( line );
			if( sStream>>t ) return true;
			else return false;
		};
		...
		unsigned int lightSamples = 100;
		RayTracingStats::Reset();
		cout << "File header: "          ; if( !ReadInput( fileHeader ) ){ WARN( "File header not provided" ) ; return; }
		cout << "File extension [jpg]: " ;      ReadInput( extension );
		cout << "Frames: "               ; if( !ReadInput( frames ) ){ WARN( "Number of frames not provided" ) ; return; }
		cout << "Duration: "             ; if( !ReadInput( duration ) ){ WARN( "Duration not provided" ) ; return; }
		cout << "Recursion depth [5]: "  ;      ReadInput( recursionDepth );
		cout << "Cut off [0.0001]: "     ;      ReadInput( cutOff );
		cout << "Light samples [100]: "  ;      ReadInput( lightSamples );
		for( unsigned int f=0 ; f<frames ; f++ )
		{
			double t = static_cast< double >(f)/(frames-1) * duration;
			scene->setCurrentTime( t , interpolationType );
			Image32 img = scene->rayTrace( _width , _height , recursionDepth , cutOff , lightSamples , false , true );
			std::string fileName = fileHeader + std::string( "." ) + std::to_string( f ) + std::string( "." ) + extension;
			img.write( fileName );
		}
		break;
	}
	case RAY_TRACE_IMAGE:	{
		string fileName;
		int recursionDepth;
		double cutOff;
	...
	case WRITE_SCENE:
	{
		...
		if( !ostream ) THROW( "Failed to open file for writing: " , fileName );
		ostream << *scene;
		break;
	}
	default:
...
void Window::View( Scene &s , int width , int height )
{
	...
	glutAddMenuEntry( " Ray-trace image " , RAY_TRACE_IMAGE );
	glutAddMenuEntry( " Ray-trace video " , RAY_TRACE_VIDEO );
